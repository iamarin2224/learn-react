...
function App() {
  ...
  const currencyInfo = useCurrencyInfo(from)  //returns as a object with key value pair, the key contains the list of currencies

  const options = Object.keys(currencyInfo) //we use only the keys in options

  //basic function to swap the currencies
  const swap = () => {
    setFrom(to)
    setTo(from)
    setAmount(convAmt)
